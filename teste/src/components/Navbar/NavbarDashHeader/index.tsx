...
const NavbarDashHeader = () => {
  ...
  async function VerificarComunicadoComercial() {
    await api
      ...
      .then((response) => {
        ...
        const comunicadosNaoLidos = comunicadosParaUsuario.filter(
          (c) =>
            !postLidos.some((p) => p.comunicadoId === c.id) &&
            !idsLidosLocal.includes(c.id)
        );
        const temAtualizacaoSistema = comunicadosParaUsuario.some(
          (c) => c?.id == 1 || c?.titulo === 'Atualizar Sistema'
        );
        const deveAtualizar = temAtualizacaoSistema && usuario.username != 'admin';

        localStorage.setItem('atualizando', deveAtualizar ? 'true' : 'false');
        setatualizando(deveAtualizar);
  ...
  const handleConfirmarComunicadoComercial = async () => {
    try {
      const postsLidos: PostLido[] = comunicadosComerciais.map((c) => ({
        comunicadoId: c.id,
        titulo: c.titulo,
        lidoEm: new Date().toISOString(),
        usuarioID: usuario.id,
      ...
      addLidosLocal(comunicadosComerciais.map((c) => c.id));
      setShowComunicadoComercial(false);
        ...
        <>
          ...
          >
            ...
            {(() => {
              ...
              const cls = isOnline && !(isApiDown && isMobile) ? 'divOline' : 'divOffline';
              return (
                <div
                  className={cls}
                  onClick={() => {
